...
from .model import LocusMasterTE, LocusMasterTELikelihood
...
def run(args):
    ...
    ''' Create LocusMasterTE object '''
    ts = LocusMasterTE(opts)
    ...
    if ts.run_info['overlap_unique'] + ts.run_info['overlap_ambig'] == 0:
        ...
        lg.info("LocusMasterTE assign complete (%s)" % fmtmins(time() - total_time))
        return
    ...
    ''' Create likelihood '''
    ts_model = LocusMasterTELikelihood(ts.raw_scores, long_read, opts)
