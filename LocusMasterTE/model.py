...
class LocusMasterTE(object):
    ...
    def update_sam(self, tl, filename):
        ...
        with pysam.AlignmentFile(self.tmp_bam, check_sq=False) as sf:
            header = sf.header
            header['PG'].append({
                'PN': 'LocusMasterTE', 'ID': 'LocusMasterTE',
    ...
    def __str__(self):
        if hasattr(self.opts, 'samfile'):
            return '<LocusMasterTE samfile=%s, gtffile=%s>'.format(
                self.opts.samfile, self.opts.gtffile)
        elif hasattr(self.opts, 'checkpoint'):
            return '<LocusMasterTE checkpoint=%s>'.format(self.opts.checkpoint)
        else:
            return '<LocusMasterTE>'


class LocusMasterTELikelihood(object):
    """
