...
def generate_test_command(args, seq_mode):
    ...
    print('lasTEq %s assign %s %s' % (seq_mode, _alnpath, _gtfpath), file=sys.stdout)
...
def main():
    ...
    bulk_parser = subparsers.add_parser('bulk',
        description='''lasTEq for bulk RNA-sequencing data sets''',
    ...
    lasTEq_assign.BulkIDOptions.add_arguments(bulk_assign_parser)
    bulk_assign_parser.set_defaults(func=lambda args: lasTEq_assign.run(args))
