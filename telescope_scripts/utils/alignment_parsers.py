...
class LocusMasterTEAlignment(object):
    """
...
class LocusMasterTERead(object):
    """
    ...
    def _make_alignments(self, segs):
        """ Logic for making LocusMasterTEAlignments from segments
        :param segs:
        :return:
        """
    ...
    def _make_unpaired(self,segs):
        ...
        for s in segs:
            self.alignments.append(LocusMasterTEAlignment(s))
    ...
    def _make_paired(self,segs):
        ...
        if self.is_proper_pair:
            ...
            for i in range(0,len(segs),2):
                self.alignments.append(LocusMasterTEAlignment(segs[i],segs[i+1]))
        else:
            ...
            # Create a primary LocusMasterTEAlignment containing both segments
            if any(s.is_unmapped for s in primary):
                ...
                self.alignments.append(LocusMasterTEAlignment(*primary))

            # Both segments have at least one alignment (could be discordant or neither)
            # Create a separate LocusMasterTEAlignment for each segment, make the better one primary
            else:
                self.is_unmapped = False
                # Create mate for primary[0] using primary[1] as template
                a1 = LocusMasterTEAlignment(primary[0], make_unmapped_mate(primary[0], template=primary[1]))
                a1.set_secondary(True)
                # Create mate for primary[1] using primary[0] as template
                a2 = LocusMasterTEAlignment(primary[1], make_unmapped_mate(primary[1], template=primary[0]))
                a2.set_secondary(True)
                self.alignments.append(a1)
                self.alignments.append(a2)

            # Make secondary LocusMasterTEAlignments for remaining alignments
            if secondary:
                ...
                for s in secondary:
                    ...
                    self.alignments.append(LocusMasterTEAlignment(s,m))
